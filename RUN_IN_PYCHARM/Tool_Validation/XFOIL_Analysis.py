...
if __name__ == "__main__":
    ...
    if flap_angle == 6:
        ...
        reference_path = os.path.join(
            tool_path, "data", "reference_values", "T1_Re0.300_M0.00_N9.0_6f_2.csv"
        )
        lowre_ref = np.loadtxt(reference_path, delimiter=";")
    else:
        reference_path = os.path.join(
            tool_path, "data", "reference_values", "T1_Re0.600_M0.00_N9.0_2.csv"
        )
        highre_ref = np.loadtxt(reference_path, delimiter=";")
        reference_path = os.path.join(
            tool_path, "data", "reference_values", "T1_Re0.300_M0.00_N9.0_2.csv"
        )
        lowre_ref = np.loadtxt(reference_path, delimiter=";")
