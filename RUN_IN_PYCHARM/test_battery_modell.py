...
class battery_model:
    ...
    def get_voltage(self, i, t):
        ...
        # Interpolate
        u_upper = interp.interp1d(batt_data_upper[:, 2], batt_data_upper[:, 3], fill_value='extrapolate', kind='linear')(soc)
        u_lower = interp.interp1d(batt_data_lower[:, 2], batt_data_lower[:, 3], fill_value='extrapolate', kind='linear')(soc)
...
if __name__ == '__main__':
    ...
    for i in range(len(t)):
        u[i], soc[i] = model1.get_voltage(i=30., t=t[i])
        u2[i], soc2[i] = model2.get_voltage(i=30., t=t[i])
    ...
    reference_data_path_2 = os.path.join(tool_path, "data", "battery_surrogates", "thrust_measurement_2400mAh.csv")
    reference_data_2 = np.loadtxt(reference_data_path_2, delimiter=",", skiprows=1)

    reference_data_path_3 = os.path.join(tool_path, "data", "battery_surrogates", "voltage_measurement_2400mAh.csv")
    reference_data_3 = np.loadtxt(reference_data_path_3, delimiter=",", skiprows=1)

    reference_data_path_4 = os.path.join(tool_path, "data", "battery_surrogates", "voltage_measurement_3000mAh.csv")
    ...
    plt.plot(reference_data_3[:, 0], reference_data_3[:, 1], label="measurement voltage 2400mAh", linestyle="--", color="red")
    plt.plot(reference_data_4[:, 0], reference_data_4[:, 1], label="measurement voltage 3000mAh", linestyle="--", color="blue")
