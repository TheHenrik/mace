...
def plot_function(
    ...
    if plot_type == "2d":
        ...
        fig = plt.figure(z_name)
        ax = plt.subplot(1, 1, 1)
        CS = ax.contourf(X, Y, Z, cmap=lightened_map, levels=np.arange(610, 990, 20)) # 600, 1000, 10
        C = ax.contour(X, Y, Z, colors="black", linewidths=0.5, levels=np.arange(610, 990, 20)) # 610, 990, 29
