...
wingsegment1 = plane.WingSegment(nose_inner=nose_inner, nose_outer=nose_outer, back_inner=back_inner,
                                 back_outer=back_outer, a_inc=a_inc, a_inc_outer=a_inc_outer,
                                 inner_airfoil=inner_airfoil, outer_airfoil=outer_airfoil)
...
# --- Propulsion ---
thrust: np.ndarray = np.array([[0, 20], [5, 19], [10, 18], [15, 16], [20, 14], [30, 10], [50, 5]])
...
name = "testplane"
mass = 5
testplane = plane.Plane(name=name, wing=wing, mass=mass, propulsion=)