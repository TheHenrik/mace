...
class EfficiencyFlight():
    ...
    def D(self, V):
        ...
        if self.is_drag_surrogate_build == False:
            ...
            drag_force = np.interp(V, v_vec, self.drag_surrogate, right = 100., left=100.)
        return drag_force
    ...
    def get_t1_min(self, v1, v2, I, h0):
        ...
        hend = self.h_end
        tend = self.t_ges
        t1_min = (tend * v2 * D(v2) + m * g * (hend - h0)) / (v1 * (T(v1,I) - D(v1) + v2 * D(v2)))
        return t1_min
