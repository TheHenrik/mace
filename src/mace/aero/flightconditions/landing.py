...
class Landing:
    ...
    def landing(self, v_touchdown, num, v_timer_end=0):
        ...
        v0 = 0
        for element in np.linspace(v_touchdown, v0, num):   # v0 = 0, v_start, Anzahl der Inkremente für Diskretisierung
            v1 = element
            v2 = element - (v_touchdown - v0) / (num - 1)
            v = element + (v_touchdown - v0) / (2 * (num-1))      # eventuell quadratisch mitteln -> rechts der Mitte
